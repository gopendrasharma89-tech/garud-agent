...
async function main(): Promise<void> {
  ...
  const { gateway, scheduler, logger, tools, metrics, broadcastChannel, longterm, dailyLog, subagent, nodes, hooks, workspace, heartbeat, embeddings, costTracker, tracer, memoryIndex, skillLibrary, heartbeatScheduler, mcpClients, hybrid, codeRunner, workflows } = await bootstrap(config);
  heartbeat.start();
  ...
  const server = createServer({
    ...
    memoryIndex, skills: skillLibrary, heartbeatScheduler, mcpClients, hybrid, codeRunner, workflows,
